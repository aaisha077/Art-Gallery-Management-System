...
if (strlen($_SESSION['agmsaid']) == 0) {
  ...
  $query = mysqli_query($con, "UPDATE tblenquiry SET AdminRemark='$remark', Status='$status', AdminRemarkdate=NOW() WHERE ID='$cid'");
  if ($query) {
    echo '<script>alert("Remarks has been updated")</script>';
    header("Location: view-enquiry-detail.php?viewid=$cid");
    exit();
  } else {
                ...
                <table border="1" class="table table-bordered mg-b-0">
                  ...
                  <tr>
                    ...
                    <td>
                      <?php
                      if (empty($row['Status'])) {
                        echo "Unanswered Enquiry";
                      } elseif ($row['Status'] == "Answer") {
                  ...
                  <?php if ($row['Status'] != "Answer") { ?>
                    <form name="submit" method="post">
                      <tr>
                        <th>Remark :</th>
                        <td>
                          <textarea name="remark" rows="5" cols="14" class="form-control wd-450" required="true"></textarea>
                        </td>
                      </tr>

                      <tr align="center">
                        <td colspan="4">
                          <button type="submit" name="submit" class="btn btn-primary btn-sm"><i class="fa fa-dot-circle-o"></i> Update</button>
                        </td>
                      </tr>
                    </form>
                  <?php } else { ?>
